...
public class ProjectADriver {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(
                "-----------------------------DRIVER BEING RUN WITH EXTRA CREDIT CLASS-----------------------------");
        ...
        // parameter 3: the values to add; values are added from the array beginning
        // (index 0) to end;
        // for example, {1, 2, 3} will first add 1 to the back, then 2 to the back, then
        // 3 to the back
        // parameter 4: expected return value (true if the element was added, false
        // otherwise)
        ...
        // parameter 3: the values to add; values are added from the array beginning
        // (index 0) to end;
        // for example, {1, 2, 3} will first add 1 to the front, then 2 to the front,
        // then 3 to the front
        // parameter 4: expected return value (true if the element was added, false
        // otherwise)
        // parameter 5: a description of the test
        ...
        testAdd(list, AddRemovePosition.FRONT, new Integer[] { 7, 9, 5, 4, 3, 8, 1 }, true,
                "addFront to fill the list");
        testIsEmptyFull(list, false, true);
        testSize(list, 10);
        ...
        // parameter 3: the expected result (true if the list contains that element,
        // false otherwise)
        ...
        // parameter 3: if it exists, this is the expected value returned from the
        // remove;
        ...
        // parameter 1: the list
        // parameter 2: the index at which you will get the element; these method calls
        ...
        testAdd(wordList, AddRemovePosition.FRONT, new String[] { "job!", "Nice", "it!", "did", "You" }, true,
                "test with Strings");
    ...
    public static <T> void testIsEmptyFull(FrontBackCappedListInterface<T> list, boolean expectedEmptyResult,
                                           boolean expectedFullResult) {
    ...
    public static <T> void testAdd(FrontBackCappedListInterface<T> list, AddRemovePosition positionToAdd,
                                   T[] valuesToAdd, boolean expectedResult, String testDescription) {
        System.out.println("\nTesting add method: trying to add " + Arrays.toString(valuesToAdd) + " to "
                + positionToAdd + " of list");
        ...
        for (T value : valuesToAdd) {
            ...
            if (actualResult != expectedResult) {
                allTestsPassed = false;
                System.out.println("**********TEST FAILED: incorrect return value when adding " + value + ".\ttest:"
                        + testDescription);
                System.out.println(
                        "Expected return result: " + expectedResult + "\n  Actual return result: " + actualResult);
            }
        }
        ...
        if (expectedResult == true) {
            ...
            if (expectedAfterSize != afterSize) {
                allTestsPassed = false;
                System.out.println("**********TEST FAILED: incorrect size after adding " + Arrays.toString(valuesToAdd)
                ...
                if (!expectedLastAddedValue.equals(actualLastAddedValue)) {
                    allTestsPassed = false;
                    System.out.println("**********TEST FAILED: incorrect list contents. Review the output below.\ttest:"
                            + testDescription);
                }
            }
        }
    ...
    public static <T> void testContains(FrontBackCappedListInterface<T> list, T element, boolean expectedResult,
    ...
    public static <T> void testIndexOf(FrontBackCappedListInterface<T> list, IndexPosition indexPosition, T element,
                                       String testDescription) {
        testIndexOf(list, indexPosition, element, -1, testDescription);
    }

    public static <T> void testIndexOf(FrontBackCappedListInterface<T> list, IndexPosition indexPosition, T element,
                                       int expectedResult, String testDescription) {
        int actualResult;
        ...
        if (expectedResult < 0 && actualResult >= 0) {
            allTestsPassed = false;
            System.out.println("**********TEST FAILED when finding the index of an element not in the list. \ttest:"
                    + testDescription);
    ...
    public static <T> void testGetEntry(FrontBackCappedListInterface<T> list, int position, T expectedResult,
                                        String testDescription) {
    ...
    public static <T> void testRemove(FrontBackCappedListInterface<T> list, AddRemovePosition positionToRemove,
                                      String testDescription) {
        testRemove(list, positionToRemove, null, testDescription);
    }

    public static <T> void testRemove(FrontBackCappedListInterface<T> list, AddRemovePosition positionToRemove,
                                      T expectedResult, String testDescription) {
        ...
        System.out
                .println("Expected returned result: " + expectedResult + "\n  Actual returned result: " + actualResult);
        ...
        if (expectedResult != null) {
            ...
            if (expectedAfterSize != actualAfterSize) {
                allTestsPassed = false;
                System.out.println("**********TEST FAILED with the wrong list size when removing from "
                        + positionToRemove + ". \ttest: " + testDescription);
                System.out.println(
