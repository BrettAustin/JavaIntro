public class LList<T extends Comparable<? super T>> implements ListInterface<T> {
    ...
    public T getMax() {
        ...
        T maxValue = firstNode.data;
        Node nodeToCheck = firstNode.next;
        while(!(nodeToCheck==null)){
            if(nodeToCheck.data.compareTo(maxValue)>0){
                maxValue=nodeToCheck.data;
            }
    ...
    public void addAll(T[] items) {
        ...
        if(numberOfItems>0){
                if(firstNode==null){//first handle if there are no Nodes
                    Node nodeToAdd = new Node(items[0]);
                    firstNode = nodeToAdd;
                    numberOfEntries++;
                    Node currentNode = firstNode;

                    for(int i = 1;i<numberOfItems;i++){
                        Node additionalNode = new Node(items[i]);
                        currentNode.next = additionalNode;
                        currentNode = currentNode.next;
                        numberOfEntries++;
                    }
                } else {
                    Node endOfCurrentChain = firstNode;
                    while (!(endOfCurrentChain.next == null)) {
                        endOfCurrentChain = endOfCurrentChain.next;
                    }
                    for (int i = 0; i < numberOfItems; i++) {
                        Node additionalNode = new Node(items[i]);
                        endOfCurrentChain.next = additionalNode;
                        endOfCurrentChain = endOfCurrentChain.next;
                        numberOfEntries++;
                    }
            }
